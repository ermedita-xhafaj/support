...
if ( $action = hesk_REQUEST('a') )
{
	...
	elseif ($action == 'order')  {order_saved();}https://www.youtube.com/watch?v=WPQUoBCxNCo
}
...
<div class="container tab-content manage-canned-tab">
	...
	<ul id="tabs" class="nav nav-tabs manage-canned" data-tabs="tabs">
		...
		{
			echo '<li id="canned-responses"><a title="' . $hesklang['manage_saved'] . '" href="manage_canned.php">' . $hesklang['manage_saved'] . '</a></li>';
		}
		?>
		<li class="active" id="ticket-templates"><a aria-controls="t-templates" role="tab" data-toggle="tab" title="<?php echo $hesklang['ticket_tpl']; ?>" href="#t-templates" onclick="javascript:alert('<?php echo hesk_makeJsString($hesklang['ticket_tpl_intro']); ?>')"><?php echo $hesklang['ticket_tpl']; ?> [?]</a></li>
	</ul>
<!-- TABS -->
...
if ($num < 1)
{
    echo '<div role="tabpanel" class="container tab-pane" id="t-templates">'.$hesklang['no_ticket_tpl'].'</div>';
}
else
{
	?>

	<div class="container tab-pane" role="tabpanel" id="t-templates">
		<table class="table-responsive table table-bordered ticket-temp-table">
			<tr>
...
<div class="container new-ticket-tpl-title"><?php echo $hesklang['new_ticket_tpl']; ?></div>
<div class="manage-ticket-templates">

		<form action="manage_ticket_templates.php" method="post" name="form1">
				<div class="add-edit-ticket-template container">
						<div>
							<?php
							if ($num > 0)
							{
								...
								<div class="radio">
									<div class="form-inline radio">
										<label class="col-sm-3 control-label"><input type="radio" name="a" value="new" <?php echo (!isset($_SESSION['canned']['what']) || $_SESSION['canned']['what'] != 'EDIT') ? 'checked="checked"' : ''; ?> /> <b><?php echo $hesklang['ticket_tpl_add']; ?></b></label><br />
									</div>
									<div class="edit-selected-tpl radio form-inline">
										<label class="col-sm-3 control-label"><input type="radio" name="a" value="edit" <?php echo (isset($_SESSION['canned']['what']) && $_SESSION['canned']['what'] == 'EDIT') ? 'checked="checked"' : ''; ?> /> <b><?php echo $hesklang['ticket_tpl_edit']; ?>:</b></label>
							...
							{
								echo '<div class="create-ticket-tpl-title"><input type="hidden" name="a" value="new" /> ' . $hesklang['ticket_tpl_add'] . '</div>';
							}
							?>

							<div class="form-inline title-template"><label class="col-sm-3 control-label title-tpl" for="title-ticket-template"><b><?php echo $hesklang['ticket_tpl_title']; ?>:</label> <span id="HeskTitle"><input class="form-control" type="text" id="title-ticket-template" name="name" size="40" maxlength="50" <?php if (isset($_SESSION['canned']['name'])) {echo ' value="'.stripslashes($_SESSION['canned']['name']).'" ';} ?> /></span></div>

							<div class="form-inline"><label class="col-sm-3 control-label msg-tpl"for="message-ticket-template"><b><?php echo $hesklang['message']; ?>:</b></label>
							<span id="HeskMsg"><textarea class="form-control" id="message-ticket-template" name="msg" rows="15" cols="70"><?php
				...
				</div><!-- end add-edit-ticket-template -->
	</div><!-- end manage-canned-tab-->
		<div class="container">
				<input type="hidden" name="token" value="<?php hesk_token_echo(); ?>" />
				<input type="submit" value="<?php echo $hesklang['save_ticket_tpl']; ?>" class="btn btn-default save-ticket-tpl-btn" />
		</div>
	</div><!--end home-canned -->	
		</form>
</div><!-- end manage-ticket-templates -->
