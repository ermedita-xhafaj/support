...
<!-- TABS -->
<div class="container tab-content" id="tools-tab">

	<ul id="tabs" class="nav nav-tabs banned-servicemsg-tools" data-tabs="tabs">
		<?php
		// Show a link to banned_emails.php if user has permission to do so
		if ( hesk_checkPermission('can_ban_emails',0) )
		{
			echo '<li id="banned-email"><a title="' . $hesklang['banemail'] . '" href="banned_emails.php">' . $hesklang['banemail'] . '</a></li> ';
		}
		...
		?>
		<li class="active" id="service-msg-tools"><a aria-controls="sm-tools" role="tab" data-toggle="tab" title="<?php echo $hesklang['sm_title']; ?>" href="#sm-tools" onclick="javascript:alert('<?php echo hesk_makeJsString($hesklang['sm_intro']); ?>')"><?php echo $hesklang['sm_title']; ?> [?]</a></li>
	</ul>
...
<div class="tab-pane active" role="tabpanel" id="sm-tools">
<?php
...
<div class="container new-smsg-title"><a name="new_article"></a><?php echo $hesklang['new_sm']; ?> [<a href="javascript:void(0)" onclick="javascript:alert('<?php echo hesk_makeJsString($hesklang['sm_intro']); ?>')">?</a>]</div>
<div class="new-service-messages-ticket">
		<?php
		...
		<form action="service_messages.php" method="post" name="form1">
			<div class="form-group new-service-messages-style-type-title">
				<div class="form-inline">
					<span class="col-sm-2" style="vertical-align: middle;"><b><?php echo $hesklang['sm_style']; ?>:</b></span>
					<label class="radio-inline none"><input type="radio" name="style" value="0" <?php if (!isset($_SESSION['new_sm']['style']) || (isset($_SESSION['new_sm']['style']) && $_SESSION['new_sm']['style'] == 0) ) {echo 'checked="checked"';} ?> /> <b><?php echo $hesklang['sm_none']; ?></b></label>
					<label class="radio-inline success"><input type="radio" name="style" value="1" <?php if (isset($_SESSION['new_sm']['style']) && $_SESSION['new_sm']['style'] == 1 ) {echo 'checked="checked"';} ?> /> <b><?php echo $hesklang['sm_success']; ?></b></label>
					<label class="radio-inline info" style="color: cadetblue;"><input type="radio" name="style" value="2" <?php if (isset($_SESSION['new_sm']['style']) && $_SESSION['new_sm']['style'] == 2) {echo 'checked="checked"';} ?> /> <b><?php echo $hesklang['sm_info']; ?></b></label>
					<label class="radio-inline notice"><input type="radio" name="style" value="3" <?php if (isset($_SESSION['new_sm']['style']) && $_SESSION['new_sm']['style'] == 3) {echo 'checked="checked"';} ?> /> <b><?php echo $hesklang['sm_notice']; ?></b></label>
					<label class="radio-inline error"><input type="radio" name="style" value="4" <?php if (isset($_SESSION['new_sm']['style']) && $_SESSION['new_sm']['style'] == 4) {echo 'checked="checked"';} ?> /> <b><?php echo $hesklang['sm_error']; ?></b></label>
				</div>
				<br/>
				<div class="form-inline">
					<span class="col-sm-2"><b><?php echo $hesklang['sm_type']; ?>:</b></span>&nbsp;
					<span id="type-smsg">
						<label class="radio-inline"><input type="radio" name="type" value="0" <?php if (!isset($_SESSION['new_sm']['type']) || (isset($_SESSION['new_sm']['type']) && $_SESSION['new_sm']['type'] == 0) ) {echo 'checked="checked"';} ?> /> <i><?php echo $hesklang['sm_published']; ?></i></label>
						&nbsp;|&nbsp;
						<label class="radio-inline" style="margin-left: 0px;"><input type="radio" name="type" value="1" <?php if (isset($_SESSION['new_sm']['type']) && $_SESSION['new_sm']['type'] == 1) {echo 'checked="checked"';} ?> /> <i><?php echo $hesklang['sm_draft']; ?></i></label><br />
					</span>
				</div>
				<br/>
				<div class="form-inline">
					<label class="col-sm-2 control-label" for="new-sm-title"><b><?php echo $hesklang['sm_mtitle']; ?>:</b></label>
					<input class="form-control" type="text" id="new-sm-title" name="title" size="70" maxlength="255" <?php if (isset($_SESSION['new_sm']['title'])) {echo 'value="'.$_SESSION['new_sm']['title'].'"';} ?> />
				</div>
			</div><!-- end new-service-messages-style-type-title -->
			<br/>
			<div class="wysiwyg-editor form-inline"><div class="col-sm-2"><b><?php echo $hesklang['sm_msg']; ?>:</b></div>
				<textarea name="message" rows="25" cols="70" id="content" style="width:48.8%; height:300px"><?php if (isset($_SESSION['new_sm']['message'])) {echo $_SESSION['new_sm']['message'];} ?></textarea>
			</div>
			<br/>
			<div class="col-sm-8 col-sm-offset-4">
				<span id="sm-save-preview">
				...
				<input type="submit" name="sm_save" value="<?php echo $hesklang['sm_save']; ?>" class="btn btn-default smsg-save-btn" />
				<input type="submit" name="sm_preview" value="<?php echo $hesklang['sm_preview']; ?>" class="btn btn-default smsg-preview-btn" />
				</span>
			</div>
			<br/>
		</form>
</div><!-- end new-service-messages-ticket -->
...
if ($num < 1)
{
    echo '<div class="container"><i>'.$hesklang['no_sm'].'</i></div><br/>';
}
else
{
	// List of staff
	...
	?>
	<div class="container ex-sm-title"><?php echo $hesklang['ex_sm']; ?></div>
		<table class="container table-responsive table table-bordered ex-sm-table">
			<tr>
