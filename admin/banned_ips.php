...
<div class="container tab-content" id="tools-tab">
	...
	<ul id="tabs" class="nav nav-tabs banned-servicemsg-tools" data-tabs="tabs">
		...
		?>
		<li class="active" id="banned-ips"><a aria-controls="b-ips" role="tab" data-toggle="tab" title="<?php echo $hesklang['banip']; ?>" href="#b-ips" onclick="javascript:alert('<?php echo hesk_makeJsString($hesklang['banip_intro']); ?>')"><?php echo $hesklang['banip']; ?> [?]</a></li>
		<?php
...
<div class="container tab-pane active" role="tabpanel" id="b-ips">
<?php
...
<form action="banned_ips.php" method="post" name="form1">
	...
	<div class="form-inline table-banned-ips">
			<label class="col-sm-2 control-label bananip-title"><?php echo $hesklang['bananip']; ?>:</label>
			<div class="col-sm-3"><input class="form-control" type="text" id="banips" name="ip" size="30" maxlength="255" />
			<input type="hidden" name="token" value="<?php hesk_token_echo(); ?>" /></div>
			<div class="col-sm-2"><input type="hidden" name="a" value="ban" />
			<input type="submit" value="<?php echo $hesklang['savebanip']; ?>" class="btn btn-default" onmouseover="hesk_btn(this,'btn btn-defaultover');" onmouseout="hesk_btn(this,'btn btn-default');" /></div>
		<span class="form-group"><i><?php echo $hesklang['banex']; ?></i><br />
			<b>123.0.0.0</b><br />
			<b>123.0.0.1 - 123.0.0.53</b><br />
			<b>123.0.0.0/24</b><br />
			<b>123.0.*.*</b>
		</span>
	</div><!-- end table-banned-ips -->
...
{
	?>
	<div class="iptemp-title"><?php echo $hesklang['iptemp']; ?></div>
		<table class="table table-bordered table-responsive iptemp-table">
			<tr>
...
if ($num < 1)
{
    echo '<br/><div>'.$hesklang['no_banips'].'</div><br/>';
}
else
{
	// List of staff
	...
	?>
	<div class="perm-ip-title"><?php echo $hesklang['ipperm']; ?></div>
		<table class="table table-bordered table-responsive perm-ip-banned">
			<tr>
