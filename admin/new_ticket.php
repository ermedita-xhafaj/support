...
<div class="container hesk-title-newTicket">
	<a href="admin_main.php"><?php echo $hesk_settings['hesk_title']; ?></a> &gt; <?php echo $hesklang['nti2']; ?>
</div>

<div class="container insert-ticekt"><?php echo $hesklang['nti2']; ?></div>

<div class="container col-sm-8 col-sm-offset-2 insert-new-ticket">
	<div>	
		<div class="nti3"><?php echo $hesklang['nti3']; ?>
			<?php echo $hesklang['req_marked_with']; ?> <font class="important">*</font>
		</div>
		<!-- START FORM -->
		<div class="container">
			<form method="post" action="admin_submit_ticket.php" name="form1" enctype="multipart/form-data">
				<!-- Contact info -->
				<div class="new-ticket-contact-info">
					<div class="form-inline new-ticket-contact-row">
						<label class="col-sm-2 control-label" for="contact-info-name"><?php echo $hesklang['name']; ?>: <font class="important">*</font></label>
						<input class="form-control contact-newTicket" type="text" id="contact-info-name" name="name" size="40" maxlength="30" value="<?php if (isset($_SESSION['as_name'])) {echo stripslashes(hesk_input($_SESSION['as_name']));} ?>" <?php if (in_array('name',$_SESSION['iserror'])) {echo ' class="isError" ';} ?> />
					</div>
					<div class="form-inline new-ticket-contact-row">
						<label class="col-sm-2 control-label" for="contact-info-email"><?php echo $hesklang['email']; ?>: <font class="important">*</font></label>
						<input class="form-control contact-newTicket" type="text" id="contact-info-email" name="email" size="40" maxlength="1000" value="<?php if (isset($_SESSION['as_email'])) {echo stripslashes(hesk_input($_SESSION['as_email']));} ?>" <?php if (in_array('email',$_SESSION['iserror'])) {echo ' class="isError" ';} elseif (in_array('email',$_SESSION['isnotice'])) {echo ' class="isNotice" ';} ?> <?php if($hesk_settings['detect_typos']) { echo ' onblur="Javascript:hesk_suggestEmail(1)"'; } ?> />
					</div>
				</div><!-- end new-ticket-contact-info -->

				<!-- Department and priority -->
				<div class="new-ticket-category-priority">
					<div class="form-inline new-ticket-contact-row">
						<label class="col-sm-2 control-label" for="new-ticket-category"><?php echo $hesklang['category']; ?>: <font class="important">*</font></label>
						<select class="form-control contact-newTicket" id="new-ticket-category" name="category" <?php if (in_array('category',$_SESSION['iserror'])) {echo ' class="isError" ';} elseif (in_array('category',$_SESSION['isnotice'])) {echo ' class="isNotice" ';} ?> >
							<?php
					...
					<div class="form-inline new-ticket-contact-row">
						<label class="col-sm-2 control-label" for="new-ticket-priority"><?php echo $hesklang['priority']; ?>: <font class="important">*</font></label>
						<select class="form-control contact-newTicket" id="new-ticket-priority" name="priority" <?php if (in_array('priority',$_SESSION['iserror'])) {echo ' class="isError" ';} ?> >
							<?php
							// Show the "Click to select"?
				...
				</div><!--end new-ticket-category-priority -->
				...
				<div class="form-group ticket-info">
						...
						<div>
							<div>
								<div class="table-responsive" >
									<table class="table table-bordered white print-templates">
										<tr>
										...
										<tr>
											...
											<td class="admin_gray">
												<label class="control-label" for="select-ticket-template"><?php echo $hesklang['select_ticket_tpl']; ?></label>:<br />
												<select class="form-control input-sm" id="select-ticket-template" name="saved_replies" onchange="setMessage(this.value)">
					...
					?>
					<div class="form-inline new-ticket-contact-row" style="padding-right: 0px;">
						<label class="col-sm-2 control-label" for="ticket-template-subject"><?php echo $hesklang['subject']; ?>: <font class="important">*</font></label> <!-- end td style="text-align:right" width="150" -->
						<span id="HeskSub"><input class="form-control contact-newTicket" type="text" id="ticket-template-subject" name="subject" id="subject" size="40" maxlength="40" value="<?php if (isset($_SESSION['as_subject'])) {echo stripslashes(hesk_input($_SESSION['as_subject']));} ?>" <?php if (in_array('subject',$_SESSION['iserror'])) {echo ' class="isError" ';} ?> /></span> <!-- delete <td width="80%"> -->
					</div>
					<div class="form-inline new-ticket-contact-row">
						<label class="col-sm-2 control-label" for="message" style=" vertical-align: top;"><?php echo $hesklang['message']; ?>: <font class="important">*</font></label> <!-- end <td style="text-align:right" width="150" valign="top"> -->
						<span id="HeskMsg"><textarea class="form-control contact-newTicket" name="message" id="message" rows="12" cols="60" <?php if (in_array('message',$_SESSION['iserror'])) {echo ' class="isError" ';} ?> ><?php if (isset($_SESSION['as_message'])) {echo stripslashes(hesk_input($_SESSION['as_message']));} ?></textarea></span> <!-- end <td width="80%"> -->
					</div>
				</div>
				...
				?>
				<div class="new-ticket-attachments">
					<div class="form-inline">
						<label class="col-sm-2 control-label"><?php echo $hesklang['attachments']; ?>:</label>
						<div class="form-group contact-newTicket">
							<?php
							for ($i=1;$i<=$hesk_settings['attachments']['max_number'];$i++)
							{
								$cls = ($i == 1 && in_array('attachments',$_SESSION['iserror'])) ? ' class="isError" ' : '';
								echo '<input id="attach-new-ticket" type="file" name="attachment['.$i.']" size="50" '.$cls.' />';
							}
							?>
							<a href="Javascript:void(0)" onclick="Javascript:hesk_window('../file_limits.php',250,500);return false;"><?php echo $hesklang['ful']; ?></a>
						</div>
					</div> <!-- end tr -->
				</div><!-- end new-ticket-attachments --> <!-- delete table border="0" width="100%" -->
				...
				?>
				<div class="new-ticket-admin-options">
					<div class="form-inline new-ticket-contact-row">
						<label class="col-sm-2 control-label"><b><?php echo $hesklang['addop']; ?>:</b></label>
						<div class="form-group contact-newTicket">
							...
							<label><input type="checkbox" name="show" value="1" <?php echo (!isset($_SESSION['as_show']) || !empty($_SESSION['as_show'])) ? 'checked="checked"' : ''; ?> /> <?php echo $hesklang['otas']; ?></label>
						</div>
					</div>
					<?php
					...
					?>
					<div class="form-inline new-ticket-contact-row">
						<label class="col-sm-2 control-label"><b><?php echo $hesklang['owner']; ?>:</b></label>
							<label class="contact-newTicket"><?php echo $hesklang['asst2']; ?><select class="form-control" id="new-ticket-select-owner" name="owner" <?php if (in_array('owner',$_SESSION['iserror'])) {echo ' class="isError" ';} ?>>
					...
					?>
					<div class="form-inline">
						<span><b><?php echo $hesklang['owner']; ?>:</b></span>
							<label><input class="contact-newTicket" type="checkbox" name="assing_to_self" value="1" <?php echo $checked; ?> /> <?php echo $hesklang['asss2']; ?></label>
					</div>
					<?php
					}
					?>
		...
		<div class="container">
			...
			<input type="submit" value="<?php echo $hesklang['sub_ticket']; ?>" class="btn btn-default submit-new-ticket-btn" />
		</div>
	</form><!-- END FORM -->
	</div>
